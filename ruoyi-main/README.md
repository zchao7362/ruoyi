...
一直想做一款后台管理系统，看了很多优秀的开源项目但是发现没有合适的。于是利用空闲休息时间开始自己写了一套后台系统。如此有了测试。她可以用于所有的Web应用程序，如网站管理后台，网站会员中心，CMS，CRM，OA。所有前端后台代码封装过后十分精简易上手，出错概率低。同时支持移动客户端访问。系统会陆续更新一些实用功能。

性别男，测试是给女儿取的名字（寓意：你若不离不弃，我必生死相依）

测试是一套全部开源的快速开发平台，毫无保留给个人及企业免费使用。
...
* 阿里云折扣场：[点我进入](http://aly.测试)，腾讯云秒杀场：[点我进入](http://txy.测试)&nbsp;&nbsp;
